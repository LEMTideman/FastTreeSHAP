...
struct PathElement {
    ...
    tfloat one_fraction;
    PathElement() {}
    PathElement(int i, tfloat z, tfloat o) :
        feature_index(i), zero_fraction(z), one_fraction(o) {}
};
...
inline void extend_path(PathElement *unique_path, tfloat *pweights, unsigned unique_depth,
...
inline void unwind_path(PathElement *unique_path, tfloat *pweights, unsigned unique_depth,
...
inline tfloat unwound_path_sum(const PathElement *unique_path, const tfloat *pweights, unsigned unique_depth,
                               unsigned unique_depth_pweights, unsigned path_index) {
    tfloat total = 0;
    const tfloat zero_fraction = unique_path[path_index].zero_fraction;
    tfloat next_one_portion = pweights[unique_depth_pweights];
...
// determine what the total permuation weight would be if
// we unwound a previous extension in the decision path
inline tfloat unwound_path_sum_zero(const tfloat *pweights, unsigned unique_depth, unsigned unique_depth_pweights) {
    tfloat total = 0;
...
inline void tree_shap_recursive(const unsigned num_outputs, const int *children_left,
    ...
    if (condition == 0 || condition_feature != static_cast<unsigned>(parent_feature_index)) {
        extend_path(unique_path, pweights, unique_depth, unique_depth_pweights, parent_zero_fraction,
    ...
    if (children_right[node_index] < 0) {
        const tfloat w_zero = unwound_path_sum_zero(pweights, unique_depth, unique_depth_pweights);
        ...
        for (unsigned i = 1; i <= unique_depth; ++i) {
            ...
            if (el.one_fraction != 0) {
               const tfloat w = unwound_path_sum(unique_path, pweights, unique_depth, unique_depth_pweights, i);
    ...
    // internal node
        ...
        if (path_index != unique_depth + 1) {
            ...
            unwind_path(unique_path, pweights, unique_depth, unique_depth_pweights, path_index);
            unique_depth -= 1;
...
inline void compute_combination_sum_recursive(const int *children_left, const int *children_right,
    ...
    // internal node
        ...
        if (path_index != unique_depth + 1) {
            duplicated_node[node_index] = path_index;
            incoming_zero_fraction = unique_path[path_index].zero_fraction;
...
// recursive computation of SHAP values for a decision tree
inline void fast_tree_shap_recursive(const unsigned num_outputs, const int *children_left,
                                     const int *children_right,
                                     const int *children_default, const int *features,
                                     const tfloat *thresholds, const tfloat *values,
                                     ...
                                     tfloat pweights_residual, tfloat parent_zero_fraction,
                                     tfloat parent_one_fraction, int parent_feature_index,
                                     int *leaf_count, int condition, unsigned condition_feature,
                                     tfloat condition_fraction) {
    ...
    if (children_right[node_index] < 0) {
        ...
        const unsigned values_offset = node_index * num_outputs;
        const tfloat scale_zero = -leaf_combination_sum[ptwos_sum] * pweights_residual * condition_fraction;
        for (unsigned i = 1; i <= unique_depth; ++i) {
            const PathElement &el = unique_path[i];
            const unsigned phi_offset = el.feature_index * num_outputs;
            const tfloat scale = (el.one_fraction != 0) ? leaf_combination_sum[ptwos_sum - ptwos[i - 1]] * \
            pweights_residual * (1 - el.zero_fraction) * condition_fraction : scale_zero;
            for (unsigned j = 0; j < num_outputs; ++j) {
                phi[phi_offset + j] += scale * values[values_offset + j];
            }
        }
    ...
    // internal node
        ...
        // divide up the condition_fraction among the recursive calls
        tfloat left_condition_fraction = condition_fraction;
        tfloat right_condition_fraction = condition_fraction;
        if (condition > 0 && split_index == condition_feature) {
            right_condition_fraction = 0;
            unique_depth -= 1;
        } else if (condition < 0 && split_index == condition_feature) {
            left_condition_fraction *= left_zero_fraction;
            right_condition_fraction *= right_zero_fraction;
            unique_depth -= 1;
        }

        fast_tree_shap_recursive(
            num_outputs, children_left, children_right, children_default, features, thresholds, values,
            ...
            split_index, leaf_count, condition, condition_feature, left_condition_fraction
        );
        ...
        fast_tree_shap_recursive(
            ...
            incoming_one_fraction * int(x[split_index] > thresholds[node_index]),
            split_index, leaf_count, condition, condition_feature, right_condition_fraction
        );
    }
}


inline void fast_tree_shap(const TreeEnsemble& tree, const tfloat *combination_sum, const int *duplicated_node,
                           const ExplanationDataset &data, tfloat *out_contribs, int condition, unsigned condition_feature) {
    ...
    // Pre-allocate space for the unique path data
    PathElement *unique_path_data = new PathElement[(tree.max_depth + 1) * (tree.max_depth + 2) / 2];
    int *ptwos = new int[tree.max_depth + 1];
    ptwos[0] = 1;
    for (unsigned i = 1; i < tree.max_depth + 1; i++) {
      ptwos[i] = ptwos[i-1] * 2;
    }
    int *leaf_count = new int[1];
    leaf_count[0] = 0;
    ...
    fast_tree_shap_recursive(
        ...
        unique_path_data, ptwos, 1, 1, 1, -1, leaf_count, condition, condition_feature, 1
    );
    ...
    if (true) {
        ...
        for (unsigned j = 0; j < trees.tree_limit; ++j) {
            ...
            for (unsigned i = 0; i < data.num_X; ++i) {
                ...
                fast_tree_shap(tree, combination_sum, duplicated_node, instance, instance_out_contribs, 0, 0);
            }
        }
